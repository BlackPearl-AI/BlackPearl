...
import { completionAuthority, goalToolExecution, requireCompletionCertification, requireDirectHuman, } from "./authority.js";
import { linkRequirementToGoal } from "./requirement.js";
...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        execute(args, exec) {
            ...
            const projectReqs = requirements.filter(r => r.source === ctx.environment.get('currentProject') ?? '');
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            properties: {
                ...
                source: { type: 'string' },
                goalId: { type: 'string', description: 'Will be null until a goal is created and linked.' },
            ...
            required: ['requirementId', 'objective', 'source', 'created'],
        },
        execute(args, exec) {
            const execution = goalToolExecution(ctx, exec);
            /** Generate proper CR-ID */
            const seq = ctx.environment.get('requirement_seq') ?? 0;
