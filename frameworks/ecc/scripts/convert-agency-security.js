...
const srcDir = 'G:/0000 PY PROGRAM/_AI_TOOLS/The Agency/security';
...
files.forEach(file => {
  ...
  if (baseSlug.startsWith('security-')) {
    baseSlug = baseSlug.replace(/^security-/, '');
  }
  const skillSlug = 'agency-security-' + baseSlug;
  
  // Extract description
  let desc = 'The Agency security specialist';
...
console.log(`Successfully converted ${count} security agents to skills & prompts!`);
