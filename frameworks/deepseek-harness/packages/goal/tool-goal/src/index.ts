...
import {
  completionAuthority,
  goalToolExecution,
  requireCompletionCertification,
  requireDirectHuman,
} from './authority.ts'
import { captureRequirement, linkRequirementToGoal, validateRequirementChain } from './requirement.ts'
import { createModule, registerModulesForGoal, getModuleRegistry } from './module.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  /** In-memory store for captured requirements in this session */
  let capturedRequirements: import('./requirement').CapturedRequirement[] = []
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args, exec) {
      ...
      // Auto-link the requirement if one was previously captured for this project
      const requirements = capturedRequirements ?? []
      const projectReqs = requirements.filter(r => r.source === ctx.environment.get<string>('currentProject') ?? '')
      for (const req of projectReqs) {
        if (req.goalId === null) {
          linkRequirementToGoal(ctx, req.id, goal.id)
        }
      }
  ...
  /** NEW: Capture user requirement and link to Universal Mapping chain */
  ctx.tools.register(defineTool({
    name: 'capture_requirement',
    description: 'Capture a user requirement with full traceability mapping into the Universal Mapping chain. Every requirement gets a CR-ID that links it to goals, modules, elements, files, dependencies, tests, and evidence. No requirement stays isolated.',
    parameters: {
      objective: {
        type: 'string',
        required: true,
        description: 'The user\'s requirement description text.',
      },
      source: {
        type: 'string',
        required: true,
        description: 'The project or domain name this requirement belongs to (e.g., "school-erp", "school-erp/student-registration").',
      },
      domain: {
        type: 'string',
        description: 'Optional domain classification (e.g., "student", "account", "attendance").',
      },
      priority: {
        type: 'string',
        enum: ['high', 'medium', 'low'],
        description: 'Optional priority level for scheduling and resource allocation.',
      },
      relatedElements: {
        type: 'array',
        items: { type: 'string' },
        description: 'Optional list of related element IDs (e.g., field names, button IDs, route paths).',
      },
      relatedFiles: {
        type: 'array',
        items: { type: 'string' },
        description: 'Optional list of related file paths that may be affected.',
      },
    },
    output: {
      schema: {
        ...
        properties: {
          ...
          goalId: {
            required: true,
            oneOf: [
              { type: 'string', description: 'Linked goal ID.' },
              { type: 'null', description: 'Will be null until a goal is created and linked.' },
            ],
          },
    ...
    execute(args, exec) {
      const execution = goalToolExecution(ctx, exec)
      /** Generate proper CR-ID */
      const seq = ctx.environment.get<number>('requirement_seq') ?? 0
      const datePart = new Date().toISOString().split('T')[0].replace(/-/g, '')
      const id = `CR-${datePart}-${String(seq + 1).padStart(3, '0')}`
      ctx.environment.set('requirement_seq', seq + 1)
      /** Create the requirement object */
      const newReq: import('./requirement').CapturedRequirement = {
        id,
        objective: args.objective,
        source: args.source,
        goalId: null,
        ...
        relatedElements: args.relatedElements || [],
        relatedFiles: args.relatedFiles || [],
        created: new Date(),
      }
      /** Add to local capturedRequirements store */
      capturedRequirements.push(newReq)
      /** Set current project in environment for later linking */
      ctx.environment.set('currentProject', args.source)
      /** Link to goal if one already exists */
      const existingGoal = ctx.goals.get(execution.agent)
      if (existingGoal && existingGoal.id) {
        const req = capturedRequirements.find(r => r.id === id)
        if (req) req.goalId = existingGoal.id
      }
      return Promise.resolve({
        requirementId: newReq.id,
        objective: newReq.objective,
        source: newReq.source,
        goalId: newReq.goalId,
        created: newReq.created.toISOString(),
        domain: newReq.domain,
        priority: newReq.priority,
        relatedElements: newReq.relatedElements,
        relatedFiles: newReq.relatedFiles,
      })
    },
    presentCall: args => present('Capture requirement', 'other', {
      objective: args.objective,
      source: args.source,
    }),
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args, exec) {
      ...
      // Autonomous completion is never trusted on the model's own assertion.
      // A direct human may still complete the goal explicitly, while a goal
      // round must pass every independently registered completion gate first.
      if (args.action === 'complete') {
        requireCompletionCertification(ctx, execution, authority)
      }
