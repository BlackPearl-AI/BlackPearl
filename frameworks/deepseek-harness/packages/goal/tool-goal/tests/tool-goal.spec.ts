...
import { registerCompletionGate } from '../src/authority.ts'
...
describe('goal tool state transitions', () => {
  ...
  it('requires independent certification before an exact goal round may complete', async () => {
    const { ctx, root } = await harness()
    const humanTurn = openTurn(root, { kind: 'user' })
    ...
    // Model assertion alone is never enough: without an independent gate,
    // autonomous completion must fail closed and leave durable goal state active.
    const withoutGate = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: created.revision, action: 'complete',
    }, root.agent)
    expect(withoutGate.error?.info?.code).toBe('GOAL_COMPLETION_GATE_MISSING')
    ...
    // A registered auditor/completion policy may explicitly deny completion.
    const disposeDeny = registerCompletionGate(
      ctx,
      () => 'completion evidence is incomplete: independent audit has not passed',
    )
    const denied = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: created.revision, action: 'complete',
    }, root.agent)
    expect(denied.error?.info?.code).toBe('GOAL_COMPLETION_NOT_CERTIFIED')
    expect(ctx.goals.get(root.agent)).toMatchObject({
      id: created.id,
      revision: created.revision,
      phase: 'active',
    ...
    // Only an independently registered gate returning no denial may certify it.
    const disposeAllow = registerCompletionGate(ctx, (execution, authority) => {
      expect(execution.agent).toBe(root.agent)
      expect(authority.goal.id).toBe(created.id)
      expect(authority.goal.revision).toBe(created.revision)
      return undefined
    })
    const complete = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: created.revision, action: 'complete',
    }, root.agent)
    expect(resultGoal(complete)).toMatchObject({
      phase: 'complete',
