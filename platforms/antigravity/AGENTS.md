...
```
                                 ...
                                 └────────────┬────────────┘
                                              ...
                                              ▼
                                 ┌─────────────────────────┐
                                 │   BlackPearl Orchestrator    │
                                 │       Supervisor        │
                                 │  - Context & Evidence   │
                                 │  - Task Routing Policy  │
                                 │  - BlackPearl Skills Engine  │
                                 └────────────┬────────────┘
                                              │
                                              ▼
                                 ┌─────────────────────────┐
...
└─────────────────┘ └─────────────────┘ └─────┬─────┘ └───────────┬──────────┘ └─────────────┘
                                              ...
                                              └─────────────┼────────────┘
                                                            ▼
                                 ┌─────────────────────────────────────┐
                                 │     BlackPearl FINAL VERIFICATION GATE   │
...
- **BlackPearl Orchestrator**: Lead Engineer & Supervisor (Task planning, constraints, routing, and final verification).
- **BlackPearl Skills**: Engineering rules, skills, patterns, and verification layer.
- **BlackPearl Core**: Real external multi-agent execution layer (Separate processes per role with worktree snapshot isolation).
...
For every real BlackPearl Core agent run, record:
...
- **TRIVIAL** (text change, tiny CSS fix, typo, simple variable rename):
  - **BlackPearl Orchestrator alone inline**. Do NOT spawn external Core agents.
- **STANDARD** (standard backend feature, API endpoint, UI component, isolated bug fix):
  - **BlackPearl Orchestrator + BlackPearl Skills discipline** inline. Optional 1 reviewer session if useful.
- **COMPLEX** (multi-file feature, large refactor, core architectural change):
  - `CORE_PLANNER ➔ CORE_IMPLEMENTER ➔ CORE_CODE_REVIEWER ➔ (CORE_FIXER if needed) ➔ BlackPearl Orchestrator Final Gate`.
- **HIGH-RISK** (authentication, authorization, payments, migrations, tenant boundaries, sensitive data):
  - `CORE_PLANNER ➔ CORE_ARCHITECT ➔ CORE_IMPLEMENTER ➔ CORE_SECURITY_REVIEWER ➔ (CORE_FIXER if needed) ➔ CORE_VERIFIER ➔ BlackPearl Orchestrator Final Gate`.
- **HEAVY** (complete module from scratch, repository-wide scan, 50+ test generation, long autonomous coding):
  - `CORE_PLANNER ➔ CORE_IMPLEMENTER (worktree) ➔ CORE_CODE_REVIEWER ➔ CORE_VERIFIER ➔ BlackPearl Orchestrator Final Gate`.
...
[CORE_IMPLEMENTER Run] ➔ [CORE_REVIEWER Run]
            ...
            │                    [ISSUE FOUND?]
            │                   /              \
            └── (YES: Fixer) ◄─                 ─► (NO) ➔ [CORE_VERIFIER] ➔ [BlackPearl GATE]
```
...
`BlackPearl Orchestrator ➔ CORE_PLANNER [run ...] ➔ CORE_IMPLEMENTER [run ...] ➔ CORE_REVIEWER [run ...] ➔ CORE_FIXER [run ...] ➔ BlackPearl Final Gate`
...
All agents (BlackPearl Orchestrator, Planners, Implementers, Reviewers, Core workers) MUST strictly enforce 5-level modular decomposition:
...
7. **Bounded Delegation**:
   - When invoking BlackPearl Core or subagents, pass strictly bounded micro-tasks (MT-ID + explicit allowed files), never open-ended module goals.

---

# 11. BlackPearl Divisions & Dynamic Multi-Agent Routing (18 Divisions, 273+ Agents)

BlackPearl Orchestrator routes to 18 specialized divisions, integrated with BlackPearl Hard Rules:
